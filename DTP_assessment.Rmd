...
##############
locale <- ggplot(data = locale_reg, aes(x = median_salary, y = index), binwidth = 30 + geom_line()+geom_point())

print(locale)
#############

urban_reg <- lm(index ~ median_salary, data = urban_df)
summary(urban_reg)

suburban_reg <- lm(index ~ median_salary, data = suburban_df)
summary(suburban_reg)

rural_reg <- lm(index ~ median_salary, data = rural_df)
summary(rural_reg)
```
